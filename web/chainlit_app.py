...
async def start():
    ...
    cl.Message(
        content="""# 🏦 Bem-vindo ao ObrasBudgetGuard - Itaú Unibanco
    ...
    try:
        ...
        if message.content.startswith("/"):
            ...
            if cmd == "upload":
                ...
                if not files:
                    await cl.Message(
                        content="❌ Nenhum arquivo foi selecionado.", author="system"
                    ).send()
                    return

                await cl.Message(
                    content=f"✅ {len(files)} arquivo(s) recebido(s). Processando..."
                ).send()
        ...
        await cl.Message(content=f"❌ Erro: {str(e)}", author="system").send()
